function addnoise_asl_mod(cleanfile, noisefile, outfile, snr) 
% ----------------------------------------------------------------------
...
[noise,srate1]= audioread(noisefile);

r1 = audioinfo(cleanfile);
nbits = r1.BitsPerSample;
r2 = audioinfo(noisefile);
nbits1 = r2.BitsPerSample;


% % if length(noise) > length(clean)
% % noise = noise(1:(length(clean)+300));
% % audiowrite('temp_noise.wav',noise,srate1,'BitsPerSample',nbits1)
% % noisefile = 'temp_noise.wav';
% % end

  if srate ~= srate1
         noise = resample(noise,srate,srate1);
         audiowrite('temp_noise.wav',noise,srate,'BitsPerSample',nbits1);
  end 
[noise,srate1]= audioread('temp_noise.wav');
...
% audioread gives floating point column data
[clean, srate]= audioread(cleanfile); 
